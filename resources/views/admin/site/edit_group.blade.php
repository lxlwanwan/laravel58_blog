                ...
                @foreach($list as $value)
                    <div class="layui-form-item" style="width: 97%;float: right">
                        <div class="layui-form-label" style="width: 100px;">
                            <input type="checkbox" name="like[]" lay-skin="primary"  lay-filter="group"  value="{{$value['id']}}" @if(in_array($value['id'],$data['p_id'])) checked="checked" @endif title="{{$value['name']}}">
                        </div>
                        <div class="layui-input-block" style="padding-top: 8px;">
                            @foreach($value['rule'] as $val)
                                <input type="checkbox" name="like2" lay-skin="primary" class="a{{$value['id']}}" @if(in_array($val['id'],$data['rules'])) checked="checked" @endif value="{{$val['id']}}" title="{{$val['name']}}">
                            @endforeach
                        </div>
                    </div>
                @endforeach
            ...
            function() {
                ...
                form.on('submit(edit)', function (data) {
                    ...
                    $.ajax({
                        ...
                        data:{id:data.field.id,name:data.field.name,rule:rule,_token:data.field._token},
                        dataType:'json',
